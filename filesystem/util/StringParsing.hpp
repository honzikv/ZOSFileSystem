...
static const std::regex DRIVE_FORMAT_REGEX = std::regex("\\d+(b|kb|mb|gb|k|m|g)");

uint64_t convertToBytes(uint64_t unitSize, std::string unit);

std::pair<uint64_t, int> extractNumber(std::string& input) {
    int lastIndex;
    auto index = 0;
...
bool matchesFormatRegex(std::string string) {
    return std::regex_match(string.begin(), string.end(), DRIVE_FORMAT_REGEX);
}


uint64_t parseDriveFormatSize(std::string input) {
